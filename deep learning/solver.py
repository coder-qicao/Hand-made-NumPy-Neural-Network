...
class Solver(object):
    ...
    def _step(self):
        ...
        #############################################################################
        # print(list(grads.values()))
        # self.log_grad_norm_history.append([np.mean(np.linalg.norm[i]) for i in list(grads.values())])
        gradient_norms = [np.linalg.norm(grad) for grad in list(grads.values())]

        # Calculate the mean of the L2 norms and store it in log_grad_norm_history
        mean_gradient_norm = np.mean(gradient_norms)
        self.log_grad_norm_history.append(mean_gradient_norm)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
